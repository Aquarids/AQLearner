...
class ShamirSecretShare(SecretShare):
    ...
    def __init__(self, num_shares, prime):
        ...
        if num_shares < 3:
            raise ValueError(
                "Require at least 3 shares for 2 out of N secret sharing")
    ...
    def _undo_convert_secret(self, secret):
        if self.secret_type == "str":
            return secret.to_bytes((secret.bit_length() + 7) // 8,
                                   'big').decode('utf-8')
        elif self.secret_type == "int":
            return secret
    ...
    def generate_shares(self, secret):
        ...
        shares = [(i, (secret + a1 * i) % self.prime)
    ...
    def recover_secret(self, shares):
        ...
        x1, y1 = shares[0]
        x2, y2 = shares[1]
        secret = (y1 * (x2 % self.prime) - y2 * (x1 % self.prime)) * pow(
            x2 - x1, -1, self.prime) % self.prime
