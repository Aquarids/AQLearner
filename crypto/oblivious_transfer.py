...
class OTSender:
    ...
    def encrypt(self, receiver_public_key):
        ...
        for secret, message in zip([secret1, secret2], self.messages):
            ...
            encrypted_messages.append((cipher.encrypt(pad(message.encode('utf-8'), AES.block_size)), cipher.iv))
...
class OTReceiver:
    ...
    def decrypt(self, encrypted_messages, sender_public_key):
        ...
        try:
            cannot_decrypt_message = unpad(cipher.decrypt(encrypted_messages[1 - self.choice][0]), AES.block_size).decode('utf-8')
        except ValueError:
