    ...
    def setDataLoader(self, train_loader, n_iters=10):
        if self.attack_type == attack_label_flip:
            if self.type == type_multi_classification:
                poisoned_loader = DataPoison.label_flip(train_loader, flip_ratio=self.kwargs["flip_ratio"], num_classes=self.kwargs["num_classes"])
            ...
            poisoned_loader = DataPoison.sample_poison(train_loader, poison_ratio=self.kwargs["poison_ratio"], noise_level=self.kwargs["noise_level"])
        elif self.attack_type == attack_ood_data:
            poisoned_loader = DataPoison.ood_data(train_loader, poison_ratio=self.kwargs["poison_ratio"])
        elif self.attack_type == attack_backdoor:
            if self.type == type_multi_classification:
                # trigger the backdoor only when the label is 1
                poisoned_loader = DataPoison.replace_label(train_loader, target_label=1, replace_label=0)
        else:
            # No attack
        ...
        if self.attack_type == attack_gradient_poison:
            ...
            for _ in range(self.n_iters):
                for X_batch, y_batch in self.train_loader:
                    ...
                    # Add gradient noise
                    ModelPoison.noise_gradient_attack(self.model, noise_level=self.kwargs["noise_level"])
            ...
            for _ in range(self.n_iters):
                for X_batch, y_batch in self.train_loader:
                    ...
                    # Add weight noise
                    ModelPoison.noise_weight_attack(self.model, noise_level=self.kwargs["noise_level"])
