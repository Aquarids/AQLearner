...
class Transaction:
    ...
    def verify(self):
        ...
        if self.signature == "system_signature":
            return True
        vk = VerifyingKey.from_string(bytes.fromhex(self.sender),
                                      curve=SECP256k1)
    ...
    def serialize(self):
        return {
            ...
            "inputs": [{
                "txid": txid,
                "index": index
            } for txid, index in self.inputs],
            "outputs": [{
                "amount": amt,
                "recipient": rec
            } for amt, rec in self.outputs],
            "timestamp":
            self.timestamp,
            "signature":
            self.signature,
    ...
    def deserialize(data):
        return Transaction(
            sender=data["sender"],
            inputs=[(input_data["txid"], input_data["index"])
                    for input_data in data["inputs"]],
            outputs=[(output["amount"], output["recipient"])
