...
class Transformer(torch.nn.Module):
    ...
    def _get_mask(self, src, tgt, padding_index):
        ...
        tgt_mask = torch.tril(torch.ones(
            tgt_seq_len, tgt_seq_len)).unsqueeze(0).unsqueeze(0)
    ...
    def fit(self, loader, learning_rate=0.01, epochs=10):
        ...
        for _ in range(epochs):
            for src, tgt in loader:
                ...
                src_mask, tgt_mask = self._get_mask(
                    src, tgt_input, padding_index=Preprocess.pad_token)
    ...
    def predict(self, loader, max_len=50):
        ...
        with torch.no_grad():
            for src, _ in loader:
                ...
                decoder_input = torch.full((curr_batch_size, 1),
                                           fill_value=Preprocess.sos_token,
                                           dtype=torch.long)
                ...
                for _ in range(max_len):
                    tgt_mask = (torch.triu(
                        torch.ones(
                            ...
                            device=src.device)) == 1).transpose(1, 2)
                    output = self.forward(src, decoder_input, src_mask,
                                          tgt_mask)
                    ...
                    tokens_eos_check = [
                        token == Preprocess.eos_token for token in tokens_list
                    ]
