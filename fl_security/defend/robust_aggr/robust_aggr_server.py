...
class RobustAggrServer(Server):
    ...
    def grad_trimmed_mean_aggr(self, grads, trim_ratio=0.1):
        ...
        for i in range(num_parameters):
            param_grads = torch.stack(
                [client_grads[i] for client_grads in grads])
            ...
            valid_indices = sorted_indices[
                num_trim:-num_trim] if num_trim > 0 else sorted_indices
    ...
    def weight_trimmed_mean_aggr(self, weights, trim_ratio=0.1):
        ...
        for key, param_group in align_weights.items():
            stacked_weights = torch.stack(param_group)
            flattened_weights = stacked_weights.view(stacked_weights.shape[0],
                                                     -1)
            ...
            trimmed_mean_reshaped = trimmed_mean.view(
                *stacked_weights.shape[1:])
    ...
    def weight_krum_aggr(self, weights, n_malicious=1):
        ...
        weight_tensors = []
        for w in weights:
            # Flatten all parameter tensors and concatenate into a single tensor
            weight_tensor = torch.cat(
                [param.data.view(-1) for param in w.values()])
        ...
        for i in range(n_clients):
            ...
            for j in range(n_clients):
                if i == j:
                    continue
                distance = torch.norm(weight_tensors[i] -
                                      weight_tensors[j]).item()
                distances.append(distance)

            distances.sort()
            # Exclude the malicious and one farthest distances
            krum_score = sum(distances[:n_clients - n_malicious - 1])
